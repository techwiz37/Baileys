...
export async function vid2jpg(videoUrl: string): Promise<string> {
	try {
		const res = await axios.get(`https://ezgif.com/video-to-jpg?url=${videoUrl}`)
		...
		const res2 = await axios.post(`https://ezgif.com/video-to-jpg/${fileToken}`, formData)

		const $2 = cheerio.load(res2.data)
		const link = $2('a.small.btn.danger[href*="/zip/"]').parent().next().find('img').first().attr('src')
...
export async function generateThumbnail(
	...
	if (mediaType === 'image') {
		...
		try {
			...
			const { buffer, original } = await extractImageThumb(imgFilename)
			thumbnail = buffer.toString('base64')
			if (original.width && original.height) {
				originalImageDimensions = {
					width: original.width,
